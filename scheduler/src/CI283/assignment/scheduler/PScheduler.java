...
/**
 * The Priority Scheduler. This scheduler takes the next process from the head of a priority queue
 * (an instance of java.util.PriorityQueue, allows it to run then puts it back into the queue (unless
 * the state of process is TERMINATED). Thus, the Priority Scheduler is identical to the Round Robin
 * scheduler apart from the fact that processes have a priority (HIGH, MED or LOW) and are held in a
 * priority queue.
 *
